...
const { getRandomRow } = require('./utils');
...
const pas = (grid, registered, i, j, col = false, depth = 0, swapped = {}) => {
    ...
    if (!swapped[`${dupe.x},${dupe.y}`]) swapped[`${dupe.x},${dupe.y}`] = true
    else if (canGet2) {
        square = col ? grid.getSquare(x + 2, y) : grid.getSquare(x, y + 2)
        isRegistered = registered[`${square.value}`] !== undefined
    } else return false
    ...
    const sorted = isRegistered ? pas(grid, registered, x, y, col, depth + 1, swapped) : true
